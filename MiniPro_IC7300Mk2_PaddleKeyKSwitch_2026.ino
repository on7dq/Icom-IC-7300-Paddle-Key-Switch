...
// 20170201 added code for long press on the 2 buttons : sets PWR to 5W or 100W
// 20230201 added more functions, using the UP/DN/LEFT/RIGHT/SELECT/START buttons
// 20260718 modified for changed CI-V commands in IC-7300 Mk2, 
//          NOTE : I still use the old transceiver address of 0x94 (change B6 in the transceiver to 94)
...
// #define adr 0xB6             // IC-7300 Mk2 new address if you want to use it
// IC7300 Serial comms
...
//                            + pull-up resistor 10k from TIP to +5V
// see circuit diagram on my blog
...
int upPin     = 2;      // UP    , sends Voice memory 1
int rightPin  = 3;      // RIGHT , sends Voice memory 2
int downPin   = 4;      // DOWN  , sends Voice memory 3
int leftPin   = 5;      // LEFT  , sends Voice memory 4
...
void loop()
{
  ...
  if (!keyPressed) { // process the 2-function buttons
    ...
    if (count > 50 && count < longTime) {
      ...
      paddle = false;
      start();
      // command 1A 05 224 00 = straight key  
      mySerial.write(0x1A);
      mySerial.write(0x05);
      mySerial.write(0x02); // (this was 01 64 for Mk1)
      mySerial.write(0x24);
    ...
    if (startButton) { // set USB output to AF
      ...
      mySerial.write(0x1A);
      mySerial.write(0x05);
      mySerial.write((byte)00);   // (this was 00 59 for Mk1)
      mySerial.write(0x69);
      mySerial.write((byte)00);
      stop();
    }
    if (selectButton) { // set USB output to IF
      ...
      mySerial.write(0x1A);
      mySerial.write(0x05);
      mySerial.write((byte)00); // (was 00 59 for Mk1)
      mySerial.write(0x69);
      mySerial.write(0x01);
      stop();
    }
    if (upButton) { // sends voice keyer memory 1
      start();
      mySerial.write(0x28);
      mySerial.write((byte)00);
      mySerial.write(0x01);
